...
/*if (isset($_SESSION['currentLoggedID']))//if the user is logged takes his id
    $id = $_SESSION['currentLoggedID'];
else if (isset($_SESSION['idGuestTest']))//if not logged takes the temporary id*/
...
try {
    ...
    //values of the firts csv segment that needs to be repeated in every line
    ...
    if ($_GET['format'] == "complete") {
        ...
        for ($i = 0; $i < $nItems; $i++) { //for every result line
            ...
            if (($i == $nItems - 1) || ($results[$i][0] != $results[$i + 1][0])  ){
                $block = $results[$i][0];
